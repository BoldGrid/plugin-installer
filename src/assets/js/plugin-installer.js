...
( function( $ ) {
  ...
  api.PluginInstaller = {
    ...
    status: function() {
      if (
        ! _bglibPluginInstaller.status &&
        ~document.referrer.indexOf( 'boldgrid-settings' )
      ) {
    ...
    _installSuccess: function() {
      $( document ).on( 'wp-plugin-install-success', function( event, response ) {
        ...
        notice
          .attr( 'class', 'notice updated-message notice-success notice-alt' )
          .find( 'p' )
    ...
    _installError: function() {
      $( document ).on( 'wp-plugin-install-error', function() {
        ...
        card
          ...
          .replaceWith(
            card.find(
              '.notice.update-message.notice-error.notice-alt.is-dismissible'
            )
          );
        card
          .find( '.notice' )
          .addClass( 'installer-messages' )
          .show();
    ...
    _processing: function() {
      $( window ).on( 'message', function( event ) {
        ...
        if ( ! _.isUndefined( message.action ) ) {
          ...
          if ( 'update-plugin' === message.action ) {
            window.tb_remove();
            ...
            el = $(
              '.bglib-plugin-installer .plugin-card-' +
                message.data.slug +
                ' .installer-messages'
            );
          ...
          if ( 'install-plugin' === message.action ) {
            window.tb_remove();
            ...
            el
              .addClass(
                'installer-messages updating-message notice inline notice-warning notice-alt'
              )
              .find( 'p' )
    ...
    _updateSuccess: function() {
      $( document ).on( 'wp-plugin-update-success', function( event, response ) {
        ...
        el
          .attr(
            'class',
            'installer-messages update-message updated-message notice inline notice-success notice-alt'
          )
          .find( 'p' )
      ...
      $.ajax( {
        ...
        success: function( response ) {
          ...
          el
            ...
            .find( '.installer-messages' )
            .addClass(
              'installer-messages notice updated-message notice-success notice-alt'
            )
            .find( 'p' )
        ...
        error: function( xhr, status, error ) {
          ...
          el
            ...
            .find( '.installer-messages' )
            .addClass(
              'installer-messages notice update-message notice-error notice-alt is-dismissible'
            )
            .find( 'p' )
            .text( error );
     ...
     */
    _buttons: function() {
      $( '.bglib-plugin-installer' ).on(
        'click',
        'a.button:not(.get-premium)',
        function( e ) {
          var el, slug;
